...
public class ModularShovel extends Item {
        ...
        if (sup.isPresent() && shup.isPresent()) {
            AttributeModifiersComponent yoShat = AttributeModifiersComponent.builder()
                    .add(
                            ...
                            new EntityAttributeModifier(Identifier.ofVanilla("base_attack_damage"), (sup.get().pickaxeDamage+shup.get().pickaxeDamage+0.5f), EntityAttributeModifier.Operation.ADD_VALUE),
                            AttributeModifierSlot.MAINHAND
                    )
                    .add(
                            EntityAttributes.ATTACK_SPEED,
                            new EntityAttributeModifier(Identifier.ofVanilla("base_attack_speed"), -3f, EntityAttributeModifier.Operation.ADD_VALUE),
                            AttributeModifierSlot.MAINHAND
                    ).build();
